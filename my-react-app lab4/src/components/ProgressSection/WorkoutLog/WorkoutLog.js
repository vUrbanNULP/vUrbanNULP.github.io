        ...
        <div id="workout-log" style={{ display: isVisible ? 'block' : 'none' }} className="workout-log">
            ...
            <ul id="log-list" className="log-list">
                {workoutLog.length === 0 ? (
                    ...
                    workoutLog.map((logEntry) => (
                        <li key={logEntry.id}> {}
                            {logEntry.workoutDate} - {logEntry.type} ({logEntry.workoutType}) - Розпочато: {logEntry.startTime}, Завершено: {logEntry.endTime}, Тривалість: {logEntry.duration}
                        </li>
                    ))
                )}
            </ul>
